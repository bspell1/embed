...
static struct {
   UI8            nPin;             // output pin
   volatile UI8   nDuty;            // current duty cycle
...
VOID PwmBangInit (PWMBANG_CONFIG* pConfig)
{
   ...
   OCR0A = F_CPU / 64 / 10000 - 1;           // reset OC0A at 25 ticks for 10kHz
}
...
{
   PwmBangSetDuty(nChannel, (UI8)(nPoint * PWMBANG_DUTY_RANGE) + PWMBANG_DUTY_MIN);
}
...
{
   static volatile UI8 g_fms = PWMBANG_CYCLE_LEN - 1;
   g_fms++;
   if (g_fms == PWMBANG_CYCLE_LEN)
   {
